...
description: plurk-mcp 的统一入口 skill。用于在多账号上下文中，在 Load Profile、Load Alerts、Load Mentions、Create Post、Create Reply 这些具体技能之间做选择，并在进入具体技能前先提醒当前工具边界和目标账号锁定要求。
---
...
- 用户同时运营多个 Plurk 账号，还没有锁定当前要操作的目标账号
...
3. 在切换前，先提醒当前边界：只支持 `plurk_get_me`、`plurk_get_alerts`、`plurk_get_mentions_context`、`plurk_get_thread_context`、`plurk_post`、`plurk_reply`，并且每次调用都要带上目标账号的 `credentials`。
4. 如果用户同时运营多个账号，但当前请求没有锁定要用哪组 `credentials`，先切到 `$plurk-mcp-load-profile` 用 `plurk_get_me` 确认账号，再继续后续工作流。
5. 如果用户请求超出边界，直接说明不支持，不要把它伪装成某个子技能可完成的任务。
